...
type FlimsySource[T any] struct {
	Read  flimsy.Getter[T]
	Write flimsy.Setter[T]
}

type benchmarkGraph struct {
	sources []*FlimsySource[int]
	layers  [][]*FlimsySource[int]
}
...
func benchmarkMakeGraph(cfg *benchmarkMakeGraphConfig) (graph *benchmarkGraph, isDynamic [][]bool) {
	rctx := &flimsy.ReactiveContext{}
	sources := make([]*FlimsySource[int], cfg.width)
	for i := range sources {
		get, set := flimsy.CreateSignal(rctx, i)
		s := &FlimsySource[int]{Read: get, Write: set}
		sources[i] = s
	}
...
func benchmarkRunGraph(cfg *benchmarkRunGraphConfig) int {
	...
	for i := 0; i < int(cfg.iteration); i++ {
		// writing signals
		flimsy.CreateMemo(cfg.rctx, func() (bool, error) {
...
func makeBenchmarkDependentRows(cfg *benchmarkMakeDependentRowsConfig) (row [][]*FlimsySource[int], isDynamic [][]bool) {
	prevRow := make([]*FlimsySource[int], len(cfg.sources))
	copy(prevRow, cfg.sources)

	random := rand.New(rand.NewSource(0))
	rows := make([][]*FlimsySource[int], cfg.numRows)
...
func makeBenchmarkRow(cfg *benchmarkRowConfig) (row []*FlimsySource[int], isDynamic []bool) {
	row = make([]*FlimsySource[int], len(cfg.sources))
	isDynamic = make([]bool, len(cfg.sources))

	for myDex := range cfg.sources {
		mySources := make([]*FlimsySource[int], 0, cfg.nSources)
		...
		if staticNode {
			...
			get, err := flimsy.CreateMemo(cfg.rctx, func() (int, error) {
				*cfg.counter++
			...
			row[myDex] = &FlimsySource[int]{Read: get}
		} else {
			first := mySources[0]
			tail := mySources[1:]
			get, err := flimsy.CreateMemo(cfg.rctx, func() (int, error) {
